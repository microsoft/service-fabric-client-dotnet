...
namespace Microsoft.ServiceFabric.Client.Http.Serialization
{
    ...
    internal class FrequencyBasedBackupScheduleDescriptionConverter
    {
        ...
        {
            var intervalType = default(BackupScheduleIntervalType?);
            var interval = default(int?);

            do
            {
                var propName = reader.ReadPropertyName();
                if (string.Compare("IntervalType", propName, StringComparison.Ordinal) == 0)
                {
                    intervalType = BackupScheduleIntervalTypeConverter.Deserialize(reader);
                }
                else if (string.Compare("Interval", propName, StringComparison.Ordinal) == 0)
                {
                    interval = reader.ReadValueAsInt();
                }
                else
                {
        ...
        internal static void Serialize(JsonWriter writer, FrequencyBasedBackupScheduleDescription obj)
        {
            ...
            writer.WriteProperty(obj.IntervalType, "IntervalType", BackupScheduleIntervalTypeConverter.Serialize);
            writer.WriteProperty(obj.Interval, "Interval", JsonWriterExtensions.WriteIntValue);
